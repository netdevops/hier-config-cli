...
def _pytest_command(
    *,
    ...
    if coverage:
        command += (
            " --cov=hier_config_cli --cov-fail-under=95 --cov-report=term-missing"
        )
    if threaded:
        command += " -n auto"
    return command
...
def _project_paths(glob: str) -> Iterable[Path]:
    # src-layout: point at the package inside src/ so the non-recursive
    # *.py glob below finds the package's modules.
    for base_dir in ("src/hier_config_cli", "tests", "scripts"):
